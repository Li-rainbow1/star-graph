# AI智绘平台 - 文生图完整流程详解

> **文档说明：** 本文档详细讲解AI智绘平台（star-graph）中"文生图"功能的完整业务流程和技术实现。  
> **项目名称：** AI智绘平台（代码包名：star-graph）

---

## 📑 目录

1. [流程概述](#流程概述)
2. [Stable Diffusion原理](#stable-diffusion原理)
3. [ComfyUI工作流](#comfyui工作流)
4. [完整业务流程10步](#完整业务流程10步)
5. [技术架构图](#技术架构图)
6. [关键技术点](#关键技术点)

---

## 流程概述

### 什么是文生图？

**文生图（Text-to-Image）** = 用户输入文字描述 → AI生成对应图片

**示例：**
- 输入："美女，沙滩，太阳伞"
- 输出：符合描述的高质量图片

### 整体流程

```
用户输入 → 参数处理 → 中译英 → 生成工作流JSON 
→ 存入Redis队列 → 定时调度 → 发送ComfyUI 
→ 监听进度 → 保存结果 → 推送前端
```

---

## Stable Diffusion原理

### 核心思想

SD生成图片的核心：**从噪声图逐步去噪，生成清晰图片**

1. 生成随机噪声图（马赛克）
2. 根据提示词逐步去除噪声
3. 得到与文本匹配的图片

### 三大组成部分

#### 1. 输入编码器（CLIP Text）
- 作用：文本 → 语义向量
- 特点：只接受英文，最多77个token
- 输出：77×768的向量

#### 2. 图片生成器（UNet + Scheduler）
- 作用：在潜空间生成图片
- 过程：迭代去噪，提升质量
- 参数：steps控制迭代次数

#### 3. 图片解码器（VAE Decoder）
- 作用：潜空间 → 像素空间
- 转换：64×64×4 → 512×512×3
- 优势：减少48倍计算量

---

## ComfyUI工作流

### 核心节点

**1. Checkpoint加载器** - 加载SD大模型

**2. CLIP文本编码器（正向）** - 正向提示词转向量

**3. CLIP文本编码器（负向）** - 负向提示词转向量

**4. 空Latent** - 创建初始潜空间

**5. K采样器** - 核心生图，迭代去噪

**6. VAE解码** - 潜空间转像素空间

**7. 保存图像** - 保存到磁盘

### 工作流JSON示例

```json
{
  "3": {
    "inputs": {
      "seed": 1097720578524313,
      "steps": 20,
      "cfg": 8,
      "sampler_name": "euler",
      "scheduler": "normal",
      "model": ["4", 0],
      "positive": ["6", 0],
      "negative": ["7", 0],
      "latent_image": ["5", 0]
    },
    "class_type": "KSampler"
  }
}
```

---

## 完整业务流程10步

### 第1步：用户提交任务

**前端操作：**
1. 输入提示词（支持中文）
2. 设置参数：数量、尺寸、步数、CFG、采样器等
3. 点击"发送"按钮

**请求参数：**
```json
{
  "propmt": "美女，沙滩，太阳伞",
  "reverse": "模糊，低质量",
  "size": 2,
  "scale": "512x768",
  "step": 20,
  "cfg": 8,
  "sampler": "euler_normal",
  "model": "majicmixRealistic_v7",
  "seed": 0,
  "clientId": "user-client-123"
}
```

---

### 第2步：参数校验与积分冻结

**Controller处理：**

```java
@PostMapping("/text2image")
public Result propmt(@RequestBody Text2ImageResDto dto) {
    // 1. 参数校验
    if (dto.getPropmt() == null) {
        throw new CustomException("提示词不能为空");
    }
    
    // 2. 检查余额
    Long userId = UserUtils.getUser().getId();
    if (balance < dto.getSize() * 10) {
        throw new CustomException("积分不足");
    }
    
    // 3. 冻结积分（每张10积分）
    userFundRecordService.freeze(userId, dto.getSize() * 10);
    
    // 4. 调用Service
    Text2ImageReqDto result = text2ImageService.propmt(dto);
    return Result.success(result);
}
```

**积分状态机：**
```
提交 → 冻结
成功 → 扣除
失败 → 归还
```

---

### 第3步：中文提示词翻译

**为什么翻译？**
- SD的CLIP模型使用英文训练
- 只接受英文输入

**使用Ollama翻译：**

```java
public String translate(String chinese) {
    OllamaRequest request = new OllamaRequest();
    request.setModel("qwen2.5:0.5b");
    request.setPrompt("翻译成英文：" + chinese);
    
    Response<OllamaResponse> response = ollamaApi.generate(request).execute();
    return response.body().getResponse();
}
```

**自动增强：**
- 正向词：`(8k, best quality, masterpiece), {翻译结果}`
- 负向词：`{翻译结果}, bad face, bad finger, bad arm, bad leg`

---

### 第4步：生成ComfyUI工作流JSON

**使用Freemarker模板引擎：**

**模板（t2i.ftlh）：**
```json
{
  "3": {
    "inputs": {
      "seed": ${config.seed},
      "steps": ${config.step},
      "cfg": ${config.cfg},
      "sampler_name": "${config.samplerName}",
      "model": ["4", 0],
      "positive": ["6", 0],
      "negative": ["7", 0]
    }
  },
  "6": {
    "inputs": {
      "text": "${config.propmt}",
      "clip": ["4", 1]
    }
  }
}
```

**Service实现：**
```java
public String renderText2Image(ComfyuiModel model) {
    Template template = configuration.getTemplate("t2i.ftlh");
    StringWriter writer = new StringWriter();
    template.process(Map.of("config", model), writer);
    return writer.toString();
}
```

---

### 第5步：封装ComfyUI任务

**对象转换：**
```
Text2ImageResDto → ComfyuiModel → 工作流JSON 
→ ComfyuiRequestDto → ComfyuiTask
```

**ComfyuiTask结构：**
```java
public class ComfyuiTask {
    private String id;           // UUID
    private String clientId;     // 前端客户端ID
    private Long index;          // 队列序号
    private String promptId;     // ComfyUI返回的ID
    private ComfyuiRequestDto comfyuiRequestDto;
    private Long userId;
    private Integer pointCost;   // 积分消耗
}
```

---

### 第6步：存储任务到Redis队列

**Redis存储结构：**

```
1. 分布式ID: DISTRIBUTED_ID (自增)
2. 任务队列: DISTRIBUTED_QUEUE (ZSet)
   - Member: 任务ID
   - Score: 队列序号
3. 任务详情: task_{任务ID} (String)
   - Value: ComfyuiTask的JSON
```

**存储代码：**
```java
public ComfyuiTask addQueueTask(ComfyuiTask task) {
    // 1. 获取队列序号
    Long idx = stringRedisTemplate.opsForValue().increment("DISTRIBUTED_ID");
    task.setIndex(idx);
    
    // 2. 存储任务详情
    stringRedisTemplate.opsForValue().set(
        "task_" + task.getId(),
        JSON.toJSONString(task)
    );
    
    // 3. 加入队列
    stringRedisTemplate.opsForZSet().add(
        "DISTRIBUTED_QUEUE",
        task.getId(),
        idx.doubleValue()
    );
    
    return task;
}
```

**响应前端：**
```json
{
  "pid": "a1b2c3-...",
  "queueIndex": 5
}
```

---

### 第7步：定时任务调度

**SpringTask每秒扫描：**

```java
@Component
public class RunTaskJob {
    
    @Scheduled(cron = "*/1 * * * * ?")
    public void task() {
        // 1. 从队列取任务
        ComfyuiTask task = redisService.popQueueTask();
        if (task == null) return;
        
        // 2. 发送给ComfyUI
        Call<HashMap> call = comfyuiApi.addQueueTask(task.getComfyuiRequestDto());
        Response<HashMap> response = call.execute();
        
        if (response.isSuccessful()) {
            String promptId = response.body().get("prompt_id").toString();
            log.info("任务提交成功：{}", promptId);
        }
    }
}
```

**取任务代码：**
```java
public ComfyuiTask popQueueTask() {
    // 弹出score最小的（优先级最高）
    TypedTuple<String> tuple = 
        stringRedisTemplate.opsForZSet().popMin("DISTRIBUTED_QUEUE");
    
    if (tuple == null) return null;
    
    // 获取任务详情
    String json = stringRedisTemplate.opsForValue()
        .getAndDelete("task_" + tuple.getValue());
    
    return JSON.parseObject(json, ComfyuiTask.class);
}
```

---

### 第8步：ComfyUI执行生图

**ComfyUI收到任务后：**

1. ✅ 加载模型
2. ✅ CLIP编码提示词
3. ✅ 创建噪声Latent
4. ✅ K采样器迭代去噪（20步）
5. ✅ VAE解码
6. ✅ 保存图片

**执行时间：**
- GPU：30秒-2分钟
- CPU：5-10分钟

---

### 第9步：WebSocket接收进度

**后端连接ComfyUI的WebSocket：**

```java
@Bean
public WebSocketConnectionManager manager(Handler handler) {
    String url = "ws://192.168.100.129:8188/ws?clientId=star-graph";
    WebSocketConnectionManager mgr = 
        new WebSocketConnectionManager(client, handler, url);
    mgr.start();
    return mgr;
}
```

**处理消息：**
```java
@Component
public class ComfyuiMessageHandler extends TextWebSocketHandler {
    
    @Override
    protected void handleTextMessage(WebSocketSession session, TextMessage msg) {
        JSONObject json = JSON.parseObject(msg.getPayload());
        String type = json.getString("type");
        
        switch (type) {
            case "progress":     // 进度更新
                handleProgress(json.getJSONObject("data"));
                break;
            case "executed":     // 生成完成
                handleExecuted(json.getJSONObject("data"));
                break;
            case "execution_error":  // 执行失败
                handleError(json.getJSONObject("data"));
                break;
        }
    }
}
```

**ComfyUI推送的消息：**

**progress消息：**
```json
{
  "type": "progress",
  "data": {
    "value": 5,
    "max": 20,
    "prompt_id": "abc123"
  }
}
```

**executed消息：**
```json
{
  "type": "executed",
  "data": {
    "output": {
      "images": [
        {"filename": "ComfyUI_00239_.png", "type": "output"}
      ]
    },
    "prompt_id": "abc123"
  }
}
```

---

### 第10步：保存结果并推送前端

**处理executed消息：**

```java
private void handleExecuted(JSONObject data) {
    // 1. 解析图片信息
    JSONArray images = data.getJSONObject("output")
                           .getJSONArray("images");
    List<String> urls = new ArrayList<>();
    
    for (int i = 0; i < images.size(); i++) {
        JSONObject img = images.getJSONObject(i);
        String filename = img.getString("filename");
        String url = buildImageUrl(filename);
        urls.add(url);
    }
    
    // 2. 保存到数据库
    Long userId = task.getUserId();
    userResultService.saveList(urls, userId);
    
    // 3. 扣除积分
    userFundRecordService.deduct(userId, urls.size() * 10);
    
    // 4. 推送给前端
    simpMessagingTemplate.convertAndSendToUser(
        task.getClientId(),
        "/topic/result",
        new ResultMessage(urls)
    );
}
```

---

## 技术架构图

```
┌─────────────┐
│  前端用户    │
└──────┬──────┘
       │ HTTP/WebSocket
       │
┌──────▼─────────────────────────┐
│     Spring Boot 后端            │
│                                │
│  ┌──────────┐  ┌────────────┐ │
│  │Controller│  │ WebSocket  │ │
│  │          │  │   服务端   │ │
│  └────┬─────┘  └─────┬──────┘ │
│       │              │        │
│  ┌────▼──────────────▼──────┐ │
│  │      Service 层          │ │
│  │  - 参数处理              │ │
│  │  - 翻译(Ollama)         │ │
│  │  - 队列管理(Redis)      │ │
│  └────┬────────────┬────────┘ │
│       │            │          │
│  ┌────▼──────┐ ┌──▼────────┐ │
│  │定时任务    │ │ WebSocket │ │
│  │(调度器)    │ │  客户端   │ │
│  └────┬──────┘ └──┬────────┘ │
└───────┼───────────┼──────────┘
        │           │
  ┌─────▼─────┐ ┌──▼────────┐
  │   Redis   │ │  ComfyUI  │
  │   队列    │ │  (AI引擎) │
  └───────────┘ └───────────┘
        │             │
  ┌─────▼─────┐ ┌────▼──────┐
  │   MySQL   │ │  Ollama   │
  │   数据库  │ │ (翻译AI)  │
  └───────────┘ └───────────┘
```

---

## 关键技术点

### 1. Redis ZSet实现优先级队列

**优势：**
- 自动按score排序
- O(log N)时间复杂度
- 支持动态调整优先级（插队）

**操作：**
```java
// 入队
zadd DISTRIBUTED_QUEUE 5 "task-123"

// 出队（获取最小score）
zpopmin DISTRIBUTED_QUEUE

// 查询排名
zrank DISTRIBUTED_QUEUE "task-123"

// 插队（减小score）
zadd DISTRIBUTED_QUEUE 2 "task-123"
```

### 2. Freemarker模板引擎

**作用：** 动态生成ComfyUI工作流JSON

**优势：**
- 模板与代码分离
- 支持条件、循环等逻辑
- 易于维护和修改

### 3. WebSocket双向通信

**两个WebSocket连接：**

**连接1：后端 → ComfyUI**
- 角色：客户端
- 用途：接收AI生图进度

**连接2：前端 → 后端**
- 角色：服务端（STOMP协议）
- 用途：推送进度给前端

**消息流转：**
```
ComfyUI → [WS客户端] → Spring Boot → [WS服务端] → 前端
```

### 4. 分布式锁（Redisson）

**解决问题：** 集群环境下定时任务重复执行

**实现：**
```java
RLock lock = redissonClient.getLock("task_scheduler_lock");
try {
    if (lock.tryLock()) {
        // 执行任务调度
        addTaskToQueue();
    }
} finally {
    if (lock.isHeldByCurrentThread()) {
        lock.unlock();
    }
}
```

### 5. @Transactional事务管理

**保证积分操作原子性：**

```java
@Transactional
public void saveList(List<String> urls, Long userId) {
    // 1. 保存图片记录
    userResultMapper.insertBatch(urls, userId);
    
    // 2. 扣除积分
    userFundRecordService.deduct(userId, urls.size() * 10);
    
    // 两步要么都成功，要么都回滚
}
```

---

## 总结

### 完整流程回顾

1. ✅ **用户提交** - 前端发送请求
2. ✅ **参数校验** - 检查参数和余额
3. ✅ **积分冻结** - 预扣积分防止刷量
4. ✅ **中译英** - Ollama翻译提示词
5. ✅ **生成JSON** - Freemarker动态渲染
6. ✅ **入队** - 存入Redis优先级队列
7. ✅ **调度** - 定时任务扫描队列
8. ✅ **发送** - 提交给ComfyUI执行
9. ✅ **监听** - WebSocket接收进度
10. ✅ **保存推送** - 结果入库并推送前端

### 核心技术栈

- **后端框架：** Spring Boot 3
- **数据库：** MySQL + MyBatis Plus
- **缓存队列：** Redis + Redisson
- **HTTP客户端：** Retrofit
- **模板引擎：** Freemarker
- **实时通信：** WebSocket + STOMP
- **AI引擎：** ComfyUI + Stable Diffusion
- **翻译AI：** Ollama (qwen2.5)

### 设计亮点

1. **优先级队列** - Redis ZSet实现，支持插队
2. **双向WebSocket** - 既是客户端又是服务端
3. **分布式锁** - 解决集群并发问题
4. **积分状态机** - 冻结→扣除/归还
5. **模板化工作流** - 易于维护和扩展

---

**文档完成时间：** 2025年10月25日  
**适用版本：** 星图AI项目 v1.0
